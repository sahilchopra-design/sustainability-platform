...
export const RiskHeatMap = ({ risks = [] }) => {
  const getColor = (impact, probability) => {
    ...
    return matrix[impact]?.[probability] || 'bg-gray-50';
  };

  const getTextColor = (impact, probability) => {
    if (impact === 'High' || impact === 'Critical') return 'text-gray-900';
    if (impact === 'Medium' && probability === 'High') return 'text-gray-900';
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6" data-testid="risk-heatmap">
      ...
      <div className="grid grid-cols-4 gap-2">
        ...
        {impactLevels.map((level) => (
          <div key={level} className="text-xs font-medium text-gray-500 text-center">
            {level}
          </div>
        ))}
              ...
              return (
                <div
                  ...
                  className={`
                    ...
                    ${risk ? getColor(impact, probability) : 'bg-gray-50'}
                    ${risk ? getTextColor(impact, probability) : 'text-gray-500'}
                  `}
