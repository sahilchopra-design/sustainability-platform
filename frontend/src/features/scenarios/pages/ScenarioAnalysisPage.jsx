...
function DashboardOverview() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500">Scenarios Created</p>
                <p className="text-2xl font-bold text-gray-900">{dashboard?.total_scenarios || 0}</p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{dashboard?.total_analyses || 0}</p>
              </div>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500">Avg Value Swing</p>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500">Top Variable</p>
      ...
      <Card>
        <CardContent className="pt-6">
          <h3 className="text-sm font-semibold text-gray-700 mb-4">Most Impactful Variables</h3>
          <div className="space-y-3">
            {(dashboard?.most_impactful_variables || []).map((v, idx) => (
              <div key={idx} className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-gradient-to-r from-violet-500 to-blue-500 flex items-center justify-center text-gray-900 text-sm font-bold">
                    {idx + 1}
                  </div>
                  <span className="font-medium text-gray-700">{v.variable}</span>
                </div>
      ...
      {dashboard?.recent_scenarios?.length > 0 && (
        <Card>
          <CardContent className="pt-6">
            <h3 className="text-sm font-semibold text-gray-700 mb-4">Recent Scenarios</h3>
            <div className="space-y-2">
              {dashboard.recent_scenarios.map((s, idx) => (
                <div key={idx} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div>
                    <div className="font-medium text-gray-700">{s.scenario_name}</div>
                    <div className="text-xs text-gray-500">{s.description || 'No description'}</div>
                  </div>
                  <Badge className={
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">Scenario Analysis</h1>
