  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap gap-2">
        ...
        <Badge label="LRF 2.2%/yr (4.2% from 2028)" color="bg-gray-50 text-gray-800" />
      ...
      <Card title="EU ETS Phase 4 — Annual Compliance Calculator" subtitle="Calculate verified emissions vs free allocation, market exposure, carbon cost, and compliance checklist status.">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
          <div className="md:col-span-2">
            <label className="block text-xs text-gray-500 mb-1">ETS Sector / Installation Type</label>
            <select className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-[#f5f6f8] text-gray-700 focus:outline-none"
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div>
            <label className="block text-xs text-gray-500 mb-1">Verified Emissions (tCO₂e)</label>
          ...
          <div>
            <label className="block text-xs text-gray-500 mb-1">Free Allocation Received (EUAs)</label>
            <input type="number" className="w-full border border-gray-200 rounded px-2 py-1.5 text-sm bg-[#f5f6f8] text-gray-700 focus:outline-none"
          ...
          <div>
            <label className="block text-xs text-gray-500 mb-1">EUA Market Price (€/tCO₂e)</label>
            <input type="number" className="w-full border border-gray-200 rounded px-2 py-1.5 text-sm bg-[#f5f6f8] text-gray-700 focus:outline-none"
        ...
        <div className="border border-gray-200 rounded-xl p-4 mb-6">
          <h3 className="text-xs font-bold text-gray-600 uppercase tracking-wide mb-3">Annual Compliance Checklist</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {COMPLIANCE_CHECKLIST.map(item => (
              <div key={item.id} className={`flex items-center gap-2 p-2 rounded-lg border cursor-pointer transition-all ${cbiAuditments[item.id] ? 'border-emerald-500/20 bg-emerald-500/[0.06]' : 'border-gray-200 bg-white/[0.01]'}`}
        ...
        <div className="space-y-6">
          ...
          <div className={`p-5 rounded-xl border ${result.netPosition < 0 ? 'border-red-500/20 bg-red-500/[0.06]' : 'border-emerald-500/20 bg-emerald-500/[0.06]'}`}>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-bold text-gray-700 mb-1">Allowance Position ({reportingYear})</p>
          ...
          {sector === 'maritime' && (
            <div className="p-4 border border-gray-200 bg-gray-50 rounded-xl">
              <p className="text-xs font-semibold text-gray-800 mb-1">Maritime ETS Phase-In ({reportingYear})</p>
      ...
      <Card title="CBAM — Carbon Border Adjustment Mechanism" subtitle="CBAM links to EU ETS from 2026. Importers of CBAM goods pay the equivalent carbon price for emissions embodied in imports.">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
          {CBAM_SECTORS.map(s => (
            <div key={s} className="flex items-center gap-2 p-2.5 border border-gray-200 rounded-lg bg-white/[0.01]">
      ...
      <Card title="Methodology Reference" className="border-gray-100">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-xs text-gray-500">
          <div><span className="font-semibold text-gray-700">Legal Base:</span> Directive 2003/87/EC (ETS) as amended</div>
          <div><span className="font-semibold text-gray-700">Phase 4 LRF:</span> 2.2%/yr (2021–2027), 4.2%/yr (2028–2030)</div>
          <div><span className="font-semibold text-gray-700">Free Alloc:</span> Benchmark-based (2021-25 and 2026-30 periods)</div>
          <div><span className="font-semibold text-gray-700">Surrender:</span> 30 April each year (Art. 12 Directive)</div>
          <div><span className="font-semibold text-gray-700">Aviation:</span> Intra-EEA + CORSIA for extra-EEA (from 2027)</div>
          <div><span className="font-semibold text-gray-700">CBAM:</span> Linked to EUA price from 2026 (Reg. 2023/956)</div>
        </div>
      </Card>
    </div>
  );
